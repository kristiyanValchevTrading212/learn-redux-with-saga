   ...
   return (
      <div className="gridElement">
         ...
         </div>
         {
            ...
            <p className="normalText" style={{ position: "absolute", marginTop: "5px" }}>
               x
               {
         ...
         <div className="normalFlex weUnder">
            ...
            {
               cartData.cartContent.find(element => element.id === id) &&
               <Button icon={faMinus} circle={true} total={true} onClick={event => dispatch(removeFromCart(id))} />
            }
            <Button icon={faShoppingBag} circle={true} total={true} onClick={event => dispatch(addToCart(productData))} />
         </div>
      </div>
   );
}
